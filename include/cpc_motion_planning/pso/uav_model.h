...
public:
  ...
  //---
  __host__ __device__
  void bound_ptcl_location(Particle &p)
  {
    for (int n = 0; n < PSO_STEPS; n++)
    {
      bound_between(p.curr_loc.site[n].x,  m_s_ini.p.x-9.0f,  m_s_ini.p.x+9.0f);
      bound_between(p.curr_loc.site[n].y,  m_s_ini.p.y-9.0f,  m_s_ini.p.y+9.0f);
      bound_between(p.curr_loc.site[n].z,  m_s_ini.p.z-9.0f,  m_s_ini.p.z+9.0f);
  ...
  //---
  __host__ __device__
  void initialize_a_particle(Particle &p)
  {
    for (int i=0; i< PSO_STEPS; i++)
    {
      p.curr_loc.site[i].x = rand_float_gen(&(p.rs), m_s_ini.p.x-6, m_s_ini.p.x+6); // station target
      p.curr_loc.site[i].y = rand_float_gen(&(p.rs), m_s_ini.p.y-6, m_s_ini.p.y+6); // station target
      p.curr_loc.site[i].z = rand_float_gen(&(p.rs), m_s_ini.p.z-6, m_s_ini.p.z+6); // station target
