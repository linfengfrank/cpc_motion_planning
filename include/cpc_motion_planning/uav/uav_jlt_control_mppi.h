...
namespace UAV
{
class UAVJLTControl_MPPI
{
public:
  UAVJLTControl_MPPI():m_theta(0)
  {
  ...
  __host__ __device__
  float3 transform_ctrl_back(const float3 &in, const float3 &cnt) {
      float3 out;

      float cos_t = cosf(cnt.z);
      float sin_t = sinf(cnt.z);

      out.x = in.x*cos_t - in.y*sin_t;
      out.y = in.x*sin_t + in.y*cos_t;
      out.z = in.z;
...
//  __host__ __device__
//  float simulate_evaluate(const EDTMap &map, const Evaluator &eva, Model &m, const PICore&sw, const typename PICore::Trace &ttr, bool &collision)
//  {
...
//  }
    ...
    __host__ __device__
    std::vector<typename Model::State> generate_trajectory(Model &m, const PICore&sw, const typename PICore::Trace &ttr)
    {
  ...
  template<class Model, class PICore>
  __host__ __device__
  typename PICore::Trace generate_control_sequence(Model &m, const PICore&sw, const float3 &wp) {
      typename PICore::Trace ctrl_sq;
      typename Model::State s = m.get_ini_state();
      float3 trans = make_float3(s.p.x, s.p.y, m_theta);
      set_ini_state(transform(s,trans));
      jlt_generate(transform(wp,trans));
      for (int i = 0; i < sw.steps; i++) {
          float t = (i+1)*sw.step_dt;
          float3 u;
          get_trajectory(s,t,u);
          ctrl_sq[i] =  transform_ctrl_back(u, trans);
      }
      return ctrl_sq;
  }
  ...
  template<class Model, class PICore>
  __host__ __device__
  std::vector<typename Model::State> generate_trajectory(Model &m, const PICore&sw, const typename PICore::Trace &ttr, float start_t = 0)
  {
    std::vector<typename Model::State> traj;
    typename Model::State s = m.get_ini_state();
    float3 trans = make_float3(s.p.x,s.p.y,m_theta);
