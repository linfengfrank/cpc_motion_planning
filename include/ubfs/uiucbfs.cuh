...
// only do the coloring. remember to clear after bfs.
__device__ void
visit_3d_node(int3 cur_buf,
              int index,
              ...
              int gray_shade, float gridstep=0.2)
{
  ...
  int cur_cost = local_map.int_cost(cur_buf.x,cur_buf.y,cur_buf.z);
  ...
  for(int i=0;i<num_dirs;i++)
  {
    ...
    int cost =cur_cost+1;
    int origin_cost = atomicMin(&local_map.int_cost(nbr_buf.x,nbr_buf.y,nbr_buf.z),cost);
                  ...
                  bool obs_mode =false)
{
  ...
  while(1){//propage through multiple BFS levels until the wavfront overgrows one-block limit
    ...
    if( tid<no_of_nodes)
    {
      ...
      visit_3d_node(pid,threadIdx.x & MOD_OP,local_q,ugraph.overflow,local_map,num_dirs,dirs,gray_shade);
    }
    __syncthreads();
    if(threadIdx.x == 0){
                           ...
                           bool obs_mode =false)
{
  ...
  while (1){//propagate through multiple levels
    ...
    if( tid<no_of_nodes_sm)
    {
      ...
      visit_3d_node(pid,threadIdx.x & MOD_OP,local_q,ugraph.overflow,
                    local_map,num_dirs,dirs,gray_shade);
    }
    __syncthreads();
           ...
           bool obs_mode =false)
{
  ...
  if( tid < no_of_nodes)
  {
    ...
    //                local_map,num_dirs,dirs,gray_shade);
    visit_3d_node(q1[tid],threadIdx.x & MOD_OP,local_q,ugraph.overflow,
                  local_map,num_dirs,dirs,gray_shade);
  }
  __syncthreads();
