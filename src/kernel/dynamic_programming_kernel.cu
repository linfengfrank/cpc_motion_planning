...
{
  CUDA_MAT::Matrix<4,action> *S_A = static_cast<CUDA_MAT::Matrix<4,action>*>(data[0]);
  CUDA_MAT::Matrix<4,float> *S_old = static_cast<CUDA_MAT::Matrix<4,float>*>(data[1]);
  CUDA_MAT::Matrix<4,float> *S_new = static_cast<CUDA_MAT::Matrix<4,float>*>(data[2]);
  ...
  float s_curr[4];
  s_curr[0] =  UGV::pos_gen_val(blockIdx.x);
  s_curr[1] =  UGV::vel_gen_val(blockIdx.y);
  s_curr[2] =  UGV::theta_gen_val(blockIdx.z);
  s_curr[3] =  UGV::w_gen_val(threadIdx.x);
...
//      val+= 0.5*acc_tot*acc_tot;
  ...
  CUDA_MAT::mat4_get_val<float>(blockIdx.x,blockIdx.y,blockIdx.z,threadIdx.x,*S_new) = val_min;

  if (updated)
    CUDA_MAT::mat4_get_val<action>(blockIdx.x,blockIdx.y,blockIdx.z,threadIdx.x,*S_A) = best_action;
...
template void GPU_DP::program<UGV::UGVModel::Input>(VoidPtrCarrier, size_t *);
