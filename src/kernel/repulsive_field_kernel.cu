...
bool find_obs_pnt(const EDTMap &map, float3 dir, const float3 &p_ini, float3 &obs_pnt)
{
  ...
  for (float i=0; i<3.0f; i+=step)
  {
    ...
    if(map.edt_const_at(ix,iy,iz).d*map.m_grid_step <= 0.6f)
    {
      stop = true;
      break;
    }
...
//---
__global__
void calc_repulsive_forces_kernel(EDTMap map,float3* rgs, UAV::UAVModel::State s, unsigned int N)
{
  ...
  if (find_obs_pnt(map, dir, s.p, obs_pnt))
  {
    if (sqrtf(dot(obs_pnt-s.p,obs_pnt-s.p)) > 0.3f)
      rgs[idx] = calc_single_repulsive_force(s, obs_pnt);
    else
      rgs[idx] = make_float3(0,0,0);
  }
  else
  {
    rgs[idx] = make_float3(0,0,0);
