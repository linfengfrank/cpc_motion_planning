...
void Pipeline::prepare_cycle()
{
  m_bb.m_init_state = prepare_init_state();
}

void Pipeline::finish_cycle()
{
...
UGV::UGVModel::State Pipeline::prepare_init_state()
{
  bool is_heading_ref;
  float psi;
...
float Pipeline::select_mes_ref(float mes, float ref, float th, int ctt_th)
{
  static int ctt = 0;
  float output;
  float err = ref - mes;
...
float Pipeline::select_mes_ref_heading(bool &is_heading_ref, float mes, float ref, float th, int ctt_th)
{
  static int ctt = 0;
  float output;
...
UGV::UGVModel::State Pipeline::predict_state(const nav_msgs::Odometry &odom, const double &psi, const int &ref_start_idx, bool is_heading_ref)
{
...
void Pipeline::full_stop_trajectory(std::vector<UGV::UGVModel::State> &traj, UGV::UGVModel::State curr_s)
{
  traj.clear();
...
bool Pipeline::is_tracking_safe(const nav_msgs::Odometry &odom, const std::vector<UGV::UGVModel::State> &ref)
{
...
float Pipeline::tracking_min_edt(const nav_msgs::Odometry &odom, const std::vector<UGV::UGVModel::State> &ref,
...
std::vector<UGV::UGVModel::State> Pipeline::simulate_tracking(const std::vector<UGV::UGVModel::State> &ref, const nav_msgs::Odometry &odom,
                                                              float yaw_ctrl_gain, float w_scale, float exam_time)
{
...
float Pipeline::traj_min_edt(const std::vector<UGV::UGVModel::State> &traj)
{
  // Find the smallest distance to obstacle
  float min_dist = 1000;
...
float Pipeline::get_dist_from_host_edt(const UGV::UGVModel::State &s) const
{
  float2 c_f,c_r;
...
void Pipeline::calculate_bounding_centres(const UGV::UGVModel::State &s, float2 &c_r, float2 &c_f) const
{
...
bool Pipeline::is_stuck(const std::vector<UGV::UGVModel::State> &traj, const UGV::UGVModel::State &tgt_state)
{
...
bool Pipeline::is_stuck_instant(const std::vector<UGV::UGVModel::State> &traj, const UGV::UGVModel::State &tgt_state)
{
...
bool Pipeline::is_stuck_lowpass(const UGV::UGVModel::State& s, const UGV::UGVModel::State &tgt_state)
{
...
bool Pipeline::is_pos_reached(const UGV::UGVModel::State &s, const UGV::UGVModel::State &tgt_state, float reaching_radius)
{
...
void Pipeline::update_reference_log(const cpc_motion_planning::ref_data &ref, const ros::Time &curr_t)
{
#ifdef PRED_STATE
  if(m_cmd_q.empty())
  {
...
std::vector<StampedUGVState> Pipeline::make_stamped_reference(int start_idx, const std::vector<UGV::UGVModel::State> & raw_ref)
{
...
void Pipeline::update_norminal_states(const std::vector<StampedUGVState> &stamped_ref)
{
...
int Pipeline::bool_to_drive_type(bool is_forward)
{
  if (is_forward)
