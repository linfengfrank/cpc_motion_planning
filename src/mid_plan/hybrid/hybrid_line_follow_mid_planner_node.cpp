...
HybridDijkstra *mid_map=nullptr;
Astar *a_map=nullptr;
...
void mapCallback(const cpc_aux_mapping::grid_map::ConstPtr& msg)
{
  ...
  if (mid_map == nullptr)
  {
    mid_map = new HybridDijkstra(msg->x_size,msg->y_size,msg->z_size);
...
//  else
    mid_map->hybrid_dijkstra_with_int_theta_with_line(local_target,seg_a,seg_b);
...
//  std::cout<<"****"<<shortest_path.size()<<std::endl;
  ...
  if (sqrtf(dot(diff,diff)) < 2.5f && current_line_id + 1 < lines.size())
  {
    current_line_id ++;
