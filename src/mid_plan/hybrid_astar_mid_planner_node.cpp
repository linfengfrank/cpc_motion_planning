...
void publishMap(int tgt_height_coord)
{
  ...
  CUDA_GEO::coord c;
  CUDA_GEO::pos p;
  for (int x=0;x<ha_map->getMaxX();x++)
  {
    for (int y=0;y<ha_map->getMaxY();y++)
    {
      for (int z=0;z<THETA_GRID_SIZE;z++)
      {
        ...
        c.y = y;
        c.z = z;
        float d_c = ha_map->hybrid_getCost2Come(c,0.0f)*15;
        if (d_c > 255) d_c = 255;
  ...
  for (int z=0;z<THETA_GRID_SIZE;z++)
  {
    for (int y=0;y<map->getMaxY();y++)
    {
      for (int x=0;x<map->getMaxX();x++)
      {
        ...
        c.y = y;
        c.z = z;
        float d_c = ha_map->hybrid_getCost2Come(c,0.0);
        tmp[i++]=d_c;
#ifdef DEBUG_COST
        mylog<<d_c<<" ";
#endif
...
void mapCallback(const cpc_aux_mapping::grid_map::ConstPtr& msg)
{
  ...
  if (ha_map == nullptr)
  {
    ...
    setup_map_msg(nf1_map_msg,ha_map,true);
  }
  else
  {
    ha_map->copyEdtData(msg);
...
void glb_plan(const ros::TimerEvent&)
{
  ...
  // Set all points
  int tgt_height_coord = ha_map->calcTgtHeightCoord(FLY_HEIGHT);

  CUDA_GEO::coord start(ha_map->getMaxX()/2,ha_map->getMaxY()/2,ha_map->getMaxZ()/2);
  start.z = tgt_height_coord;

  CUDA_GEO::coord glb_tgt = ha_map->pos2coord(goal);
  glb_tgt.z = tgt_height_coord;
  glb_tgt = ha_map->rayCast(start,glb_tgt).back();
  ...
  float length = 0.0f;
  std::vector<HybridAstar::path_info> path = ha_map->plan(curr_pose,glb_tgt);
  cpc_motion_planning::path path_cell = ha_map->split_path(path);

//  setup_map_msg(nf1_map_msg,ha_map,false);
//  copy_map_to_msg(nf1_map_msg,ha_map,tgt_height_coord);
...
//  CUDA_GEO::pos mid_goal_pos = ha_map->coord2pos(local_target);
...
//  std::vector<float3> shortest_path = ha_map->get_shortest_path(curr_pose);
//  std::cout<<"****"<<shortest_path.size()<<std::endl;
//#ifdef DEBUG_COST
//  std::ofstream mylog;
//  mylog.open("/home/sp/test/path.txt");
//#endif
//  for (size_t i=0;i<path.size();i++)
//  {
//    pcl::PointXYZRGB clrP;
//    clrP.x = path[i].pose.x;
//    clrP.y = path[i].pose.y;
//    clrP.z = 0;
//    pclOut->points.push_back (clrP);

//#ifdef DEBUG_COST
//        mylog<<path[i].pose.x<<" "<<path[i].pose.y<<" "<<path[i].pose.z<<" "<<path[i].action.x<<" "<<path[i].action.y<<" "<<path[i].action.z<<std::endl;
//#endif
//  }
//#ifdef DEBUG_COST
//  mylog.close();
