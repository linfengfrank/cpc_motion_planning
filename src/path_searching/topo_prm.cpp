...
//            nh.param("topo_prm/sample_inflate_y", sample_inflate_(1), 5.0);
//            nh.param("topo_prm/sample_inflate_z", sample_inflate_(2), 1.0);
//            nh.param("topo_prm/clearance", clearance_, 0.5);
//            nh.param("topo_prm/short_cut_num", short_cut_num_, 1);
//            nh.param("topo_prm/reserve_num", reserve_num_, 6);
//            nh.param("topo_prm/ratio_to_short", ratio_to_short_, 5.5);
//            nh.param("topo_prm/max_sample_num", max_sample_num_, 2000);
//            nh.param("topo_prm/max_sample_time", max_sample_time_, 0.007);
//            nh.param("topo_prm/max_raw_path", max_raw_path_, 300);
//            nh.param("topo_prm/max_raw_path2", max_raw_path2_, 25);
//            nh.param("topo_prm/parallel_shortcut", parallel_shortcut_, true);
//  resolution_ = edt_environment_->sdf_map_->getResolution();
            resolution_ = edt_env_->getGridStep();//edt_environment_->sdf_map_->getResolution();
//  offset_ = Eigen::Vector3d(0.5, 0.5, 0.5) - edt_environment_->sdf_map_->getOrigin() / resolution_;
            CUDA_GEO::pos _edt_origin = edt_env_->getOrigin();
            Eigen::Vector3d origin_(_edt_origin.x, _edt_origin.y, _edt_origin.z);
            offset_ = Eigen::Vector3d(0.5, 0.5, 0.5) -
                       origin_/ resolution_;//edt_environment_->sdf_map_->getOrigin() / resolution_;
        ...
        void TopologyPRM::findTopoPaths(Eigen::Vector3d start, Eigen::Vector3d end,
                                        ...
                                        vector <vector<Eigen::Vector3d>> &filtered_paths1,
            ...
            auto filtered_paths = pruneEquivalent(short_paths_);

            filtered_paths_ = filtered_paths;
            filtered_paths1 = filtered_paths;
...
//    dist = edt_environment_->evaluateCoarseEDT(pt, -1.0);
                dist = edt_env_->distAt(pt(0), pt(1), pt(2), -1.0)*edt_env_->getGridStep();//edt_environment_->evaluateCoarseEDT(pt, -1.0);
                if (dist <= clearance_) {
        ...
        vector <GraphNode::Ptr> TopologyPRM::findVisibGuard(Eigen::Vector3d pt) {
            ...
            for (list<GraphNode::Ptr>::iterator iter = graph_.begin(); iter != graph_.end(); ++iter) {
                ...
                if (lineVisib(pt, (*iter)->pos_, visible_check_scale*resolution_, pc)) { //2.5
        ...
        bool TopologyPRM::lineVisib(const Eigen::Vector3d &p1, const Eigen::Vector3d &p2, double thresh,
            ...
            while (casters_[caster_id].step(ray_pt)) {
                pt_id(0) = std::floor(ray_pt(0) + offset_(0));
                pt_id(1) = std::floor(ray_pt(1) + offset_(1));
                pt_id(2) = std::floor(ray_pt(2) + offset_(2));
//    dist = edt_environment_->sdf_map_->getDistance(pt_id);
                CUDA_GEO::coord _s(pt_id(0), pt_id(1), pt_id(2));
                dist = edt_env_->distAt(_s, -1.0)*edt_env_->getGridStep();//ironment_->sdf_map_->getDistance(pt_id);
                if (dist <= thresh) {
//      edt_environment_->sdf_map_->indexToPos(pt_id, pc);
                    CUDA_GEO::coord _edt_pt_id(pt_id(0), pt_id(1), pt_id(2));
                    CUDA_GEO::pos _edt_pc;
                    _edt_pc = edt_env_->indexToPos(_edt_pt_id);
                    pc << _edt_pc.x, _edt_pc.y, _edt_pc.z;
                    return false;
                }
            }
            return true;
        ...
        vector <vector<Eigen::Vector3d>> TopologyPRM::pruneEquivalent(vector <vector<Eigen::Vector3d>> &paths) {
            ...
            for (int i = 1; i < paths.size(); ++i) {
                ...
                for (int j = 0; j < exist_paths_id.size(); ++j) {
                    ...
                    bool same_topo = sameTopoPath(paths[i], paths[exist_paths_id[j]], 0.0); //0.0
        ...
        void TopologyPRM::shortcutPath(vector <Eigen::Vector3d> path, int path_id, int iter_num) {
            ...
            for (int k = 0; k < iter_num; ++k) {
                ...
                for (int i = 1; i < dis_path.size(); ++i) {
                    if (lineVisib(short_path.back(), dis_path[i],  visible_check_scale*resolution_, colli_pt, path_id)) continue; //2.5

//      edt_environment_->evaluateEDTWithGrad(colli_pt, -1, dist, grad);
                    CUDA_GEO::pos edt_colli_pt(colli_pt(0), colli_pt(1), colli_pt(2));
                    CUDA_GEO::pos edt_grad;
                    edt_env_->distWithGrad(edt_colli_pt, -1.0, dist, edt_grad);
                    grad << edt_grad.x, edt_grad.y, edt_grad.z;
                    if (grad.norm() > 1e-3) {
                        grad.normalize();
        ...
        void TopologyPRM::shortcutPaths() {
            ...
            if (parallel_shortcut_) {
                ...
                for (int i = 0; i < raw_paths_.size(); ++i) {
                    short_threads.push_back(thread(&TopologyPRM::shortcutPath, this, raw_paths_[i], i, 3)); // 1
                }
                for (int i = 0; i < raw_paths_.size(); ++i) {
                    short_threads[i].join();
                }
            } else {
                for (int i = 0; i < raw_paths_.size(); ++i) shortcutPath(raw_paths_[i], i, 3); // 1
            }
        }
...
//void TopologyPRM::setEnvironment(const EDTEnvironment::Ptr& env) { this->edt_environment_ = env; }
        ...
        void TopologyPRM::getPathsVisualizationMsg(visualization_msgs::MarkerArray& msg) {
            vector<vector<Eigen::Vector3d>> visual_paths = filtered_paths_;//raw_paths_;//short_paths_;//filtered_paths_;// final_paths_;//
            msg.markers.clear();
            std::string base_name = "topology_roadmap";
            visualization_msgs::Marker visual_path_msg;
            visual_path_msg.header.frame_id = "/world";
            visual_path_msg.header.stamp = ros::Time::now();
            visual_path_msg.type = visualization_msgs::Marker::LINE_STRIP;
            visual_path_msg.action = visualization_msgs::Marker::ADD;
            visual_path_msg.pose.position.x = 0;
            visual_path_msg.pose.position.y = 0;
            visual_path_msg.pose.position.z = 0;
            visual_path_msg.pose.orientation.w = 1;
            visual_path_msg.pose.orientation.x = 0;
            visual_path_msg.pose.orientation.y = 0;
            visual_path_msg.pose.orientation.z = 0;
            visual_path_msg.scale.x = 0.1;
            visual_path_msg.scale.y = 0.1;
            visual_path_msg.scale.z = 0.1;
            for (int i = 0; i < visual_paths.size(); i++) {
                visualization_msgs::Marker tmp_msg = visual_path_msg;
                tmp_msg.ns = base_name;// + std::to_string(i+1);
                tmp_msg.id = i+1;
                float color_scale = static_cast<float>(i) / static_cast<float>(visual_paths.size());
                tmp_msg.color.r = color_scale;
                tmp_msg.color.g = 1.0 - color_scale;
...
//                tmp_msg.lifetime = ros::Duration(0.1);
                for (int j = 0; j < visual_paths[i].size(); j++) {
                    ...
                    tmp_msg.points[j].z = visual_paths[i][j](2);
                }
                msg.markers.push_back(tmp_msg);
            }

            CUDA_GEO::pos _edt_origin = edt_env_->getOrigin();
            float res = edt_env_->getGridStep();
            visualization_msgs::Marker map_msg = visual_path_msg;
            map_msg.ns = "map_edt";
            map_msg.id = 1;
            map_msg.scale.x = res;
            map_msg.scale.y = res;
            ...
            map_msg.color.a = 1.0;
            map_msg.color.r = 1.0;
            map_msg.color.g = 0.45;
            map_msg.color.b = 0.0;
            map_msg.type = visualization_msgs::Marker::CUBE_LIST;
            ...
            map_msg.points.clear();
            seen_msg.points.clear();
//            Eigen::Vector3d origin(_edt_origin.x, _edt_origin.y, _edt_origin.z);
            for (int i = 0; i < edt_env_->getMaxX(); i++) {
                for (int j = 0; j < edt_env_->getMaxY(); j++) {
                    for (int k = 0; k < edt_env_->getMaxZ(); k++) {
                        ...
                        if (edt_env_->distAt(edt_p.x, edt_p.y, edt_p.z, -1)*edt_env_->getGridStep()< res) {
            ...
            msg.markers.push_back(map_msg);
            msg.markers.push_back(seen_msg);
        }
// TopologyPRM::
    }  // namespace fast_planner
}
